...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!expectedKey) {
      console.error('[RegistrationApprove] CRITICAL: ADMIN_API_KEY not configured')
    ...
    if (!adminKey || adminKey !== expectedKey) {
      console.warn('[RegistrationApprove] Unauthorized request - invalid or missing API key')
      return NextResponse.json(
